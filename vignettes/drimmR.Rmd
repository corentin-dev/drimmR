...
Let $\bm{\Pi}_0 = (\Pi_0(u, v))_{u,v \in E}$ and
$\bm{\Pi}_1 = (\Pi_1(u, v))_{u,v \in E}$ be two Markov transition
...
$\bm{\Pi}_0$ and $\bm{\Pi}_1$ if the distribution of $X_t,$
...
Let us denote by $\bm{\alpha} = (\alpha(1), \ldots, \alpha(s))$ the
...
$\bm{\Pi}_{\frac{i}{d}} = (\bm{\Pi}_{\frac{i}{d}}(u_1, \dots, u_k, v))_{u_1, \dots, u_k,v \in E}$
...
$i, j \in \{0, 1, \ldots, d\},$ $A_i\g(\frac{nj}{d}\e)=\mi_{\{i=j\}}.$*
:::

We would like to stress that the coherence between notations implies the
choice of the notation $\bm{\Pi}_{\frac{i}{d}}$ and that, in fact, $A_i$
...
accordingly (e.g., $\bm{\Pi}_0^{UU},$ $\bm{\Pi}_0^{DU},$
$\bm{\alpha}^{U}$ etc.).
...
order 1) $\bm{\Pi}_0$ and $\bm{\Pi}_1,$ for any states $u, v \in E,$ the
...
{P_5(H, \bm{m}, \bm{n})P_1(H, \bm{m}, \bm{n}) - P_3(H, \bm{m}, \bm{n})^2},\end{aligned}$$
